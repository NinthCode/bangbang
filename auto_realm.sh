...
backup_existing_config() {
    ...
    echo "  -> existing Realm config backed up to $BACKUP_FILE"
}
...
migrate_existing_config() {
    ...
    echo "▶ 检测到已有 Realm 配置，但没有 endpoints.db。"
    echo "  -> 将尝试导入简单的 [[endpoints]] listen/remote 配置。"
    ...
    if ! awk '
        ...
        /^[[:space:]]*use_udp[[:space:]]*=[[:space:]]*true/ && !in_endpoint {
            global_udp = "yes"
            next
        }
        ...
        in_endpoint && /^[[:space:]]*listen[[:space:]]*=/ {
            listen = read_string($0)
            next
        }
        in_endpoint && /^[[:space:]]*remote[[:space:]]*=/ {
            remote = read_string($0)
            next
        }
        in_endpoint && /use_udp[[:space:]]*=[[:space:]]*true/ {
            udp = "yes"
            next
        }
        in_endpoint {
    ...
    ' "$CONFIG_FILE" > "$TMP_DB"; then
        rm -f "$TMP_DB"
        echo "❌ 无法安全导入已有配置，已停止以避免覆盖 $CONFIG_FILE。"
        echo "   请先手动备份/迁移配置，或删除旧配置后重新运行脚本。"
        return 1
    fi
