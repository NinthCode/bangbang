...
echo "=========================================================="
echo "       Alpine Shadowsocks 轻量代理配置管家"
echo "=========================================================="
echo ""
...
require_alpine() {
    ...
    . /etc/os-release
    if [ "$ID" != "alpine" ]; then
        echo "❌ 当前脚本仅支持 Alpine Linux，当前系统为: $ID"
        exit 1
    fi
...
command_args="--single-threaded -c $CONFIG_FILE"
...
restart_service() {
    ...
    if ! rc-service "$SERVICE_NAME" status >/dev/null 2>&1; then
        ...
        echo "请执行以下命令查看错误："
        echo "  /usr/bin/ssserver --single-threaded -c $CONFIG_FILE -v"
        exit 1
    fi
}
